...
import { getCoffeeOptions } from "@/lib/db" // Import server-side data fetching
import { CoffeeMenuSection } from "@/components/coffee-menu-section" // Import the new client component
...
export default async function LandingPage() {
  let coffeeOptions = []
  let loadingCoffee = true
  try {
    coffeeOptions = await getCoffeeOptions()
    loadingCoffee = false
  } catch (err) {
    console.error("Failed to load coffee options on server:", err)
    // In a real app, you might want to show a fallback UI or error message
    // For now, we'll just pass an empty array and keep loadingCoffee as true
    // to indicate an issue, or handle it in the client component.
    // For a server component, it's better to let the error boundary catch it
    // or return a specific error state. For this example, we'll just log.
    loadingCoffee = false // Set to false so the client component doesn't show "Loading..." indefinitely
  }
  ...
  return (
    <PageWrapper className="min-h-screen bg-background">
      ...
      {/* Coffee Menu Preview */}
      <CoffeeMenuSection coffeeOptions={coffeeOptions} loadingCoffee={loadingCoffee} />
